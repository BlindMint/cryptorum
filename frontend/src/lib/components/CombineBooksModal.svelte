<script lang="ts">
	...
	let { bookIds, onClose, onCombined, variant = 'books' }: Props = $props();
	...
	let isAudio = $derived(variant === 'audio');
	...
	async function loadBooks(ids: number[]) {
		...
		try {
			...
			for (const id of ids) {
				...
				if (!res.ok) {
					error = isAudio ? 'Unable to load one of the selected audio records.' : 'Unable to load one of the selected books.';
					continue;
				}
			...
			if (!error && uniqueLibraryIds(loaded).length > 1) {
				error = isAudio ? 'Audio records must belong to the same library.' : 'Books must belong to the same library.';
			}
		} catch (e) {
			console.error('Failed to load books for combine:', e);
			error = isAudio ? 'Unable to load the selected audio records.' : 'Unable to load the selected books.';
		} finally {
			loading = false;
		}
	}
	...
	function progressLabel(book: CombineBookSummary): string {
		if (book.status === 'reading' || (book.percent || 0) > 0) {
			return `${Math.round(book.percent || 0)}% ${isAudio ? 'played' : 'read'}`;
		}
		if (book.status === 'finished') return 'Finished';
		return isAudio ? 'Unplayed' : 'Unread';
	}
	...
	async function confirmCombine() {
		...
		try {
			...
			if (!res.ok) {
				error = payload.error || (isAudio ? 'Failed to group audiobook.' : 'Failed to combine books.');
				return;
			}
			onCombined(payload.primary_book_id || primaryBookId);
		} catch (e) {
			console.error(isAudio ? 'Failed to group audiobook:' : 'Failed to combine books:', e);
			error = isAudio ? 'Failed to group audiobook.' : 'Failed to combine books.';
		} finally {
			saving = false;
		}
	}
...
<div class="fixed inset-0 z-[11000] flex items-center justify-center p-4">
	...
	<div class="relative flex max-h-[90dvh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-[var(--color-surface-border)] bg-[var(--color-surface-overlay)] shadow-2xl">
		<div class="flex items-start justify-between gap-4 border-b border-[var(--color-surface-border)] px-5 py-4">
			<div>
				<h2 class="text-lg font-semibold text-[var(--color-surface-text)]">{isAudio ? 'Group audiobook' : 'Combine books'}</h2>
				<p class="mt-1 text-sm text-[var(--color-surface-text-muted)]">
					{isAudio
						? 'Choose the primary audiobook. The other records become additional files on it and are removed from the Audio library. Files on disk are not deleted.'
		...
		<div class="min-h-0 flex-1 overflow-y-auto p-5">
			{#if loading}
				<div class="flex items-center justify-center py-10 text-[var(--color-surface-text-muted)]">
					...
					{isAudio ? 'Loading selected audio…' : 'Loading selected books…'}
				</div>
			{:else}
				<div class="space-y-2">
					{#each books as book (book.id)}
						<label class="flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-3 transition-colors {primaryBookId === book.id ? 'border-[var(--color-primary-500)] bg-[var(--color-primary-500)]/10' : 'border-[var(--color-surface-border)] hover:border-[var(--color-surface-500)]'}">
							...
							<BookCoverFrame
								...
								format={book.format}
								mode="cover"
								placeholderKind={isAudio ? 'audio' : 'book'}
							...
							<div class="min-w-0 flex-1">
								...
								<div class="truncate text-xs text-[var(--color-surface-text-muted)]">{parseAuthors(book.authors) || (isAudio ? 'Unknown artist' : 'Unknown author')}</div>
		...
		<div class="flex items-center justify-end gap-2 border-t border-[var(--color-surface-border)] px-5 py-4">
			...
			>
				{saving ? (isAudio ? 'Grouping…' : 'Combining…') : (isAudio ? 'Group Audiobook' : 'Combine')}
			</button>
		</div>
	</div>
</div>
