<script lang="ts">
	...
	function getFilteredMetadata() {
		...
		return metadata.filter(item => lenientSearchMatch(searchQuery, item.name));
	}
...
<div class="space-y-6">
	...
	<div class="border-b border-[var(--color-surface-border)]">
		<nav class="metadata-tab-scroll flex overflow-x-auto overflow-y-hidden">
			{#each tabs as tab}
				<button
...
<style>
	...
	.metadata-tab-scroll::-webkit-scrollbar:vertical {
		display: none;
		width: 0;
	}
