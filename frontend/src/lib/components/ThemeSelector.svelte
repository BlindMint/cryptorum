<script lang="ts">
	...
	function ignoreOutsideClick() {
		// The parent mobile actions panel owns outside-tap dismissal in inline mode.
	}
	...
	{#if showDropdown}
		<div
			class={panelOnly
				? 'w-full p-4'
				: mobileMenu
					? 'fixed left-3 right-3 top-[7.75rem] max-h-[calc(100dvh-8.5rem)] overflow-y-auto bg-[var(--color-surface-overlay)] backdrop-blur-sm border border-[var(--color-surface-border)] rounded-lg shadow-lg z-[95] p-4'
