		...
		{#if mobileActionsView === 'menu'}
			<div class="grid gap-1.5 p-2.5">
				...
				<button type="button" class="mobile-action-row w-full" onclick={openMobileTheme}>
					<span class="flex items-center gap-2.5">
						<svg class="h-4 w-4 text-[var(--color-surface-text-muted)]" viewBox="0 -960 960 960" fill="currentColor">
				...
				<a
					...
					class="mobile-action-link flex items-center gap-2.5 rounded-md px-2.5 py-1.5 text-sm text-[var(--color-surface-text)] transition-all"
				>
					<svg class="h-4 w-4 text-[var(--color-surface-text-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
					</svg>
...
<style>
	.mobile-actions-panel {
		position: fixed;
		right: 0.75rem;
		top: calc(var(--app-topbar-height) + 0.35rem);
		z-index: 90;
		width: min(22rem, calc(100vw - 1.5rem));
		max-height: calc(100dvh - var(--app-topbar-height) - 1rem);
		overflow: hidden;
