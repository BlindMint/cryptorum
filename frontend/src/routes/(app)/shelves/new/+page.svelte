<script lang="ts">
	...
	function addCondition() {
		conditions.push({ field: 'status', operator: 'equals', value: 'unread' });
	}
	...
	async function createMagicShelf() {
		...
		try {
			const response = await fetch('/api/shelves', {
				...
				body: JSON.stringify({
					...
					icon: shelfIcon,
					is_magic: 1,
					rules_json: JSON.stringify({ conditions }),
					sort_by: 'added_at',
					sort_dir: 'desc'
				})
			});
			...
			if (response.ok) {
				...
				console.error('Failed to create magic shelf');
			}
		} catch (error) {
			console.error('Error creating magic shelf:', error);
		} finally {
			isSubmitting = false;
		}
	}
...
<div class="space-y-6">
	<div>
		<h1 class="text-2xl font-bold text-[var(--color-surface-text)]">Create Magic Shelf</h1>
		<p class="text-[var(--color-surface-text-muted)] mt-1">Automatically organize books based on rules</p>
	</div>
			...
			<div>
				...
				<div class="space-y-3">
					{#each conditions as condition, index}
						<div class="flex items-center space-x-3 p-3 bg-[var(--color-surface-overlay)] rounded-lg border border-[var(--color-surface-border)]">
				...
				<p class="text-xs text-[var(--color-surface-text-muted)] mt-2">
					All conditions must be met (AND logic). Books will be automatically added/removed from this shelf.
				</p>
			</div>
