<script lang="ts">
  ...
  let serverHasMore = $state(false);
	...
	function openCombineModal() {
		const error = combineSelectionError(selectedBooks.size, selectAllMode === 'filtered');
		if (error) return;
		...
		<div class="sticky top-0 z-30 px-3 py-3 transition-all duration-200 sm:px-6 sm:py-4 {toolbarScrolled ? 'border-b border-[var(--color-surface-border)] bg-[var(--color-surface-base)]/80 shadow-[0_1px_0_rgba(255,255,255,0.04),0_10px_24px_rgba(0,0,0,0.16)] backdrop-blur-md' : 'border-b border-transparent bg-transparent shadow-none backdrop-blur-0'} {showFilterPanel ? 'lg:pr-[var(--filter-panel-offset)]' : ''}">
				...
				<div class="grid min-w-0 grid-cols-[minmax(0,1fr)_2.5rem_auto_auto] items-center gap-2">
					...
					>
						...
						{#if showSettingsMenu}
							<div class="floating-surface absolute right-0 top-full z-40 mt-2 w-56 max-w-[calc(100vw-1.5rem)] max-h-[70vh] overflow-y-auto rounded-lg border py-3">
					...
					{#if showSortMenu}
						<div class="floating-surface absolute right-0 top-full z-40 mt-2 w-48 rounded-lg border py-1">
					...
					<div class="flex items-center space-x-2">
						<div class="relative">
							...
							<button
								...
								disabled={actionInProgress || !!combineSelectionError(selectedBooks.size, selectAllMode === 'filtered')}
								title={combineSelectionError(selectedBooks.size, selectAllMode === 'filtered') || 'Combine selected books into one'}
								class="px-4 py-2 text-sm rounded-lg bg-[var(--color-surface-700)] hover:bg-[var(--color-surface-600)] text-[var(--color-surface-text)] font-medium transition-all duration-200 ease-out hover:-translate-y-px hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary-500)] disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none flex items-center space-x-2"
							>
								<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
