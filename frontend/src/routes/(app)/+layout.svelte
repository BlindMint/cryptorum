...
{#if loading}
	<div class="min-h-screen bg-[var(--color-surface-base)] flex items-center justify-center overflow-x-hidden">
		<div class="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--color-primary-500)]"></div>
	</div>
{:else if authenticated}
	<div class="h-screen bg-transparent relative overflow-x-hidden">
		...
		{#if !isReaderPage}
			<TopBar />
		{/if}
		<div class="flex {isReaderPage ? 'h-dvh' : 'h-[calc(100dvh-4rem)]'} relative min-w-0 overflow-x-hidden">
			{#if $mobileMenuOpen}
				<button
					type="button"
