...
func cbzContentType(ext string) string {
...
func getCbzManifest(filePath string) (*cbzManifest, error) {
	...
	cbzManifestCache.RLock()
	cached := cbzManifestCache.items[filePath]
	if cached != nil && cached.size == size && cached.modTime == modTime {
		cbzManifestCache.RUnlock()
		return cached, nil
	}
	cbzManifestCache.RUnlock()
	...
	entries := make([]cbzManifestEntry, 0, len(reader.File))
	for _, f := range reader.File {
		if f.FileInfo().IsDir() || strings.Contains(f.Name, "__MACOSX/") {
			continue
		}
		contentType := cbzContentType(filepath.Ext(f.Name))
		if contentType == "" {
			continue
		}
		entries = append(entries, cbzManifestEntry{
			Name:             f.Name,
	...
	manifest := &cbzManifest{
		size:    size,
		modTime: modTime,
		entries: entries,
	}
	cbzManifestCache.Lock()
	cbzManifestCache.items[filePath] = manifest
	cbzManifestCache.Unlock()
...
func countCbzPages(filePath string) (int, error) {
	manifest, err := getCbzManifest(filePath)
	if err != nil {
		return 0, err
	}
...
func ServeBookFileHandler(w http.ResponseWriter, r *http.Request) {
	...
	serveFileWithRange(w, r, filePath)
}
...
func ServeCbxPageHandler(w http.ResponseWriter, r *http.Request) {
	...
	switch format {
	case "cbz":
		serveCbzPage(w, r, filePath, pageNum)
	case "cbr":
...
func serveCbzPage(w http.ResponseWriter, r *http.Request, filePath string, pageNum int) {
	manifest, err := getCbzManifest(filePath)
	if err != nil {
	...
	etag := fmt.Sprintf("\"cbz-%d-%d-%d-%08x\"", manifest.size, manifest.modTime, pageNum, entry.CRC32)
	...
	if entry.UncompressedSize > 0 {
		w.Header().Set("Content-Length", strconv.FormatUint(entry.UncompressedSize, 10))
	}
	_, _ = io.Copy(w, rc)
}
