...
INSERT INTO book_metadata (book_id, title, authors, series, series_number, series_number_display, publisher, pub_date, description, rating, genres, isbn, asin, cover_path, cover_updated_on, page_count, language, locked_fields)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);

-- name: UpdateBookMetadata :exec
UPDATE book_metadata SET title = ?, authors = ?, series = ?, series_number = ?, series_number_display = ?, publisher = ?, pub_date = ?, description = ?, rating = ?, genres = ?, isbn = ?, asin = ?, cover_path = ?, cover_updated_on = ?, page_count = ?, language = ?, locked_fields = ?
WHERE book_id = ?;
